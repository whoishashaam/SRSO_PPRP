...
{
    public class BetaFamilyController : Controller
    {
        private readonly IConfiguration _configuration;
        private readonly ILogger<BetaFamilyController> _logger;

        public BetaFamilyController(IConfiguration configuration, ILogger<BetaFamilyController> logger)
        {
        ...
        // Updated GenerateReportData method to deduplicate UUIDs and household members
        private ReportViewModel GenerateReportData(string villageId)
        {
            ...
            string connectionString = _configuration.GetConnectionString("DefaultConnection");
            var processedUuids = new HashSet<string>(); // To track unique UUIDs
            ...
            try
            {
                ...
                using (OracleConnection conn = new OracleConnection(connectionString))
                {
                    ...
                    // Fetch distinct UUIDs from PSC_SERVEY_SCORE (ensure no duplicates at this level)
                    string uuidQuery = @"
                        SELECT DISTINCT s.UUID, s.TOTAL_PSC_SCORE
                        FROM NRSP.PSC_SERVEY_SCORE s
                        WHERE s.VILLAGE_ID = :villageId
                        AND s.UUID IN (SELECT UUID FROM HH_MM_DATA)";
                    using (OracleCommand cmd = new OracleCommand(uuidQuery, conn))
                    {
                        ...
                        using (OracleDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                // Skip if UUID has already been processed
                                if (!processedUuids.Add(uuid)) // Add returns false if UUID already exists
                                {
                                    _logger.LogWarning("Duplicate UUID {Uuid} detected, skipping", uuid);
                                    continue;
                                }

                                _logger.LogInformation("Processing unique UUID: {Uuid} from PSC_SERVEY_SCORE", uuid);

                                // Fetch members for this UUID and deduplicate them
                                string memberQuery = @"
                                ...
                                {
                                    memberCmd.Parameters.Add(new OracleParameter("uuid", uuid));
                                    _logger.LogInformation("Executing member query for UUID: {Uuid}", uuid);
                                    using (OracleDataReader memberReader = memberCmd.ExecuteReader())
                                    {
                                        ...
                                        // Use a HashSet to track unique members within this household
                                        var uniqueMembers = new HashSet<string>();

                                        while (memberReader.Read())
                                        {
                                            string name = memberReader["NAME"].ToString();
                                            string gender = memberReader["GENDER"].ToString() == "1" ? "M" : "F";
                                            string age = memberReader["AGE_YEARS"].ToString();
                                            string contactNo = memberReader["CONTACT_NO"].ToString();
                                            bool isHead = memberReader["HEAD"].ToString() == "1";
                                            string address = memberReader["ADDRESS"].ToString();

                                            // Create a unique key for the member to detect duplicates
                                            string memberKey = $"{name.ToLower()}|{gender}|{age}|{isHead}";

                                            // Skip if this member has already been added
                                            if (!uniqueMembers.Add(memberKey))
                                            {
                                                _logger.LogWarning("Duplicate member {Name} (Gender: {Gender}, Age: {Age}, IsHead: {IsHead}) in UUID {Uuid}, skipping",
                                                    name, gender, age, isHead, uuid);
                                                continue;
                                            }
                                        ...
                                        // Only add the household if it has members
                                        if (household.Members.Any())
                                        {
                                            reportModel.Households.Add(household);
                                            _logger.LogInformation("Added household with UUID {Uuid} to report (Members: {MemberCount})",
                                                uuid, household.Members.Count);
                                        }
                                        else
                                        {
                                            _logger.LogWarning("Household with UUID {Uuid} has no members after deduplication, skipping", uuid);
                                        }
                                    }
                                }
                            }
                ...
                _logger.LogInformation("Generated report data with {HouseholdCount} unique households for villageId: {VillageId}",
                    reportModel.Households.Count, villageId);
                return reportModel;
            }
            catch (Exception ex)
            {
        ...
        private byte[] GeneratePdfReport(ReportViewModel model)
        {
            try
            {
                ...
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    {
                        // Skip if no members (shouldn't happen due to prior checks, but added as a safeguard)
                        if (!household.Members.Any())
                        {
                            _logger.LogWarning("Household with UUID {Uuid} has no members during PDF generation, skipping", household.UUID);
                            continue;
                        }

                        _logger.LogInformation("Adding household {Index} to PDF", householdIndex);
                        document.Add(new Paragraph($"Number {householdIndex++}"));
