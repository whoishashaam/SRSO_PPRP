...
namespace SRSO_PPRP.Controllers
{
    public class FamilyController : Controller
    {
        ...
        [HttpPost]
        public IActionResult DownloadPSCReportByEnumeratorV2(PSCReportFilterViewModel model)
        {
            if (string.IsNullOrEmpty(HttpContext.Session.GetString("User")))
            {
                return RedirectToAction("Login", "Main");
            ...
            {
                TempData["Message"] = "Please select all filters (District, Tehsil, UC, Revenue Village, Village, and Enumerator).";
                return RedirectToAction("DownloadPSCReportByEnumeratorV2");
            }
            ...
            try
            {
                ...
                WHERE UPPER(cv.VILLAGE_NAME) = UPPER(:village) 
                        ...
                        using (OracleDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                pscScores.Add(new PSCServeyScore
                                {
                                    ...
                                    VILLAGE_NAME = reader["VILLAGE_NAME"]?.ToString(),
                                    HOUSEHOLD_MEMBERS_COUNT_SCORE = reader["HOUSEHOLD_MEMBERS_COUNT_SCORE"]?.ToString(),
                                    TOILET_TYPE = reader["TOILET_TYPE"]?.ToString(),
                                    ROOMS = reader["ROOMS"]?.ToString(),
                                    REFRIGERATOR = reader["REFRIGERATOR"]?.ToString(),
                                    FREEZER = reader["FREEZER"]?.ToString(),
                                    WASHING_MACHINE = reader["WASHING_MACHINE"]?.ToString(),
                                    GEYSER = reader["GEYSER"]?.ToString(),
                                    HEATER = reader["HEATER"]?.ToString(),
                                    AIR_COOLER = reader["AIR_COOLER"]?.ToString(),
                                    AIR_CONDITIONER = reader["AIR_CONDITIONER"]?.ToString(),
                                    COOKING_STOVE = reader["COOKING_STOVE"]?.ToString(),
                                    COOKING_RANGE = reader["COOKING_RANGE"]?.ToString(),
                                    MICROWAVE_OVEN = reader["MICROWAVE_OVEN"]?.ToString(),
                                    MOTORCYCLE = reader["MOTORCYCLE"]?.ToString(),
                                    TRACTOR = reader["TRACTOR"]?.ToString(),
                                    CAR = reader["CAR"]?.ToString(),
                                    LIVESTOCK_SCORE = reader["LIVESTOCK_SCORE"]?.ToString(),
                                    AREA_UNIT = reader["AREA_UNIT"]?.ToString(),
                                    AREA_UNIT_PSC = reader["AREA_UNIT_PSC"]?.ToString(),
                                    LAND_AREA_VALUE = reader["LAND_AREA_VALUE"]?.ToString(),
                                    LAND_TYPE = reader["LAND_TYPE"]?.ToString(),
                                    HEAD_EDUCATION_SCORE = reader["HEAD_EDUCATION_SCORE"]?.ToString(),
                                    TOTAL_PSC_SCORE = reader["TOTAL_PSC_SCORE"]?.ToString(),
                                    CREATED_DATE = Convert.ToDateTime(reader["CREATED_DATE"]),
                                    CELL_PHONE = reader["CELL_PHONE"]?.ToString(),
                    ...
                    // Fetch household data for all UUIDs, including contact number for household head
                    string hhQuery = @"
                SELECT 
                    hh.ID,
                    hh.UUID,
                ...
                WHERE UPPER(cv.VILLAGE_NAME) = UPPER(:village) 
                    ...
                    using (var memoryStream = new MemoryStream())
                    {
                        ...
                        foreach (var pscScore in pscScores)
                        {
                            ...
                            // Get contact number for household head
                            var headContactNo = householdData.FirstOrDefault(m => m.Head == "1")?.ContactNo ?? "N/A";
                            ...
                            document.Add(new Paragraph("UCP: Household PSC V2")
                                .SetFont(font)
                                .SetFontSize(headerFontSize)
                                .SetBold()
                                .SetTextAlignment(TextAlignment.LEFT)
                            ...
                            document.Add(new Paragraph($"Contact Number (Head): {headContactNo}")
                                .SetFont(font)
                                .SetFontSize(smallFontSize)
                                .SetTextAlignment(TextAlignment.LEFT)
                                .SetMarginBottom(1));
                            ...
                            // Map numeric values to their corresponding labels
                            string toiletLabel = pscScore.TOILET_TYPE switch
                            {
                                "1" => "Flush",
                                "2" => "Dry Raised/Pit Latrine",
                                "3" => "No Toilet",
                            ...
                            string refrigeratorLabel = pscScore.REFRIGERATOR switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string freezerLabel = pscScore.FREEZER switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string washingMachineLabel = pscScore.WASHING_MACHINE switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string geyserLabel = pscScore.GEYSER switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string heaterLabel = pscScore.HEATER switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string airCoolerLabel = pscScore.AIR_COOLER switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string airConditionerLabel = pscScore.AIR_CONDITIONER switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string cookingStoveLabel = pscScore.COOKING_STOVE switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string cookingRangeLabel = pscScore.COOKING_RANGE switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string microwaveOvenLabel = pscScore.MICROWAVE_OVEN switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string motorcycleLabel = pscScore.MOTORCYCLE switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string tractorLabel = pscScore.TRACTOR switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string carLabel = pscScore.CAR switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string agriLandLabel = pscScore.LAND_TYPE switch
                            {
                                "1" => "Yes",
                                "2" => "No",
                                _ => "N/A"
                            ...
                            string unitlabel = pscScore.AREA_UNIT switch
                            {
                                "1" => "Kanal",
                                "2" => "Acre",
                                "3" => "Sqfeet",
                            ...
                            //string agriLandLabel = pscScore.LAND_AREA_VALUE != "0" ? "Yes" : "No";

                            var assets = new List<(string Label, string Value)>
                    {
                        ("Rooms #", pscScore.ROOMS ?? "N/A"),
                        ("Latrine Type", toiletLabel),
                        ("Refrigerator", refrigeratorLabel),
                        ("Freezer", freezerLabel),
                        ("Cell Phone", cellPhoneLabel),
                        ...
                        ("Mules/Horse #", pscScore.MULE_HORSE ?? "N/A"),
                        ("Agri Land", agriLandLabel),
                        ("Agri Land Unit", unitlabel),
                        ("Agri Land Area", pscScore.LAND_AREA_VALUE ?? "N/A"),
                        ("GPS-Latitude", pscScore.LATITUDE ?? "N/A"),
                        ("GPS-Longitude", pscScore.LONGITUDE ?? "N/A"),
                        ("GPS-Altitude", "0.00000000000m"),
                        ("GPS-Accuracy", "4.00000000000m")
                    };
                            ...
                            var memberHeaders = new List<string>
                    {
                        ...
                        "B.Cert", "CNIC", "CNIC #", "School", "Education", "Disability", "Occupation", "Religion"
                            ...
                            int memberIndex = 1;
                            foreach (var member in householdData.OrderBy(m => m.Relation == "1" ? 0 : 1).ThenBy(m => m.Name))
                            {
                                string genderLabel = member.Gender switch
                                {
                                    "1" => "Male",
                                ...
                                memberTable.AddCell(new Cell().Add(new Paragraph(relationLabel)));
                                memberTable.AddCell(new Cell().Add(new Paragraph(maritalStatusLabel)));
                                memberTable.AddCell(new Cell().Add(new Paragraph("N/A")));
                                memberTable.AddCell(new Cell().Add(new Paragraph(member.CNICStatusID == "1" ? "YES, Available" : "YES, Not Available")));
                                memberTable.AddCell(new Cell().Add(new Paragraph(string.IsNullOrEmpty(member.CNIC) ? "N/A" : member.CNIC)));
                                memberTable.AddCell(new Cell().Add(new Paragraph(string.IsNullOrEmpty(member.CNIC) ? "N/A" : member.CNIC)));
                                memberTable.AddCell(new Cell().Add(new Paragraph("N/A")));
                        ...
                        var fileBytes = memoryStream.ToArray();
                        return File(fileBytes, "application/pdf", $"PSC_Report_V2_{model.SelectedEnumerator}_{model.SelectedVillage}.pdf");
                    }
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error generating PSC report V2 for village: {model.SelectedVillage}, enumerator: {model.SelectedEnumerator}");
                TempData["Message"] = "An error occurred while generating the report. Please try again.";
                return RedirectToAction("DownloadPSCReportByEnumeratorV2");
            }
        }
        ...
        [HttpGet]
        public IActionResult DownloadPSCReportByEnumeratorV2()
        {
            if (string.IsNullOrEmpty(HttpContext.Session.GetString("User")))
            {
                return RedirectToAction("Login", "Main");
            ...
            try
            {
                ...
                _logger.LogInformation($"Fetched {model.Districts.Count} districts for PSC report V2.");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error fetching districts for PSC report V2.");
                TempData["Message"] = "An error occurred while loading districts. Please try again.";
                return View("DownloadPSCReportByEnumeratorV2", model); // Return the V2 view
            }

            return View("DownloadPSCReportByEnumeratorV2", model);
        }

        [HttpGet]
        public JsonResult GetPSCTehsilsV2(string district)
        {
            var tehsils = new List<SelectListItem>();
            string connectionString = _configuration.GetConnectionString("DefaultConnection");
            try
            {
                ...
                _logger.LogInformation($"Fetched {tehsils.Count} tehsils for district: {district} in V2.");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error fetching tehsils for district: {district} in V2.");
                throw; // Re-throw the exception to let the client-side handle it
            }
            return Json(tehsils);
        }
        ...
        [HttpGet]
        public JsonResult GetPSCUCsV2(string tehsil)
        {
            var ucs = new List<SelectListItem>();
            string connectionString = _configuration.GetConnectionString("DefaultConnection");
            try
            {
                ...
                _logger.LogInformation($"Fetched {ucs.Count} union councils for tehsil: {tehsil} in V2.");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error fetching union councils for tehsil: {tehsil} in V2.");
                throw; // Re-throw the exception to let the client-side handle it
            }
            return Json(ucs);
        }
        ...
        [HttpGet]
        public JsonResult GetPSCRVsV2(string uc)
        {
            var rvs = new List<SelectListItem>();
            string connectionString = _configuration.GetConnectionString("DefaultConnection");
            try
            {
                ...
                _logger.LogInformation($"Fetched {rvs.Count} revenue villages for UC: {uc} in V2.");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error fetching revenue villages for UC: {uc} in V2.");
                throw; // Re-throw the exception to let the client-side handle it
            }
            return Json(rvs);
        }
        ...
        [HttpGet]
        public JsonResult GetPSCVillagesV2(string rv)
        {
            var villages = new List<SelectListItem>();
            string connectionString = _configuration.GetConnectionString("DefaultConnection");
            try
            {
                ...
                _logger.LogInformation($"Fetched {villages.Count} villages for revenue village: {rv} in V2.");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error fetching villages for revenue village: {rv} in V2.");
                throw; // Re-throw the exception to let the client-side handle it
            }
            return Json(villages);
        }
        ...
        [HttpGet]
        public JsonResult GetPSCEnumeratorsV2(string village)
        {
            var enumerators = new List<SelectListItem>();
            string connectionString = _configuration.GetConnectionString("DefaultConnection");
            try
            {
                ...
                _logger.LogInformation($"Fetched {enumerators.Count} enumerators for village: {village} in V2.");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error fetching enumerators for village: {village} in V2.");
                throw;
            }
            return Json(enumerators);
        }
